...
rofi_launcher = spawn "rofi -no-lazy-grab -show drun -modi drun -theme $HOME/.config/rofi/launcher/style -drun-icon-theme \"candy-icons\""
...
increaseVolume = spawn "pactl set-sink-volume 0 +5% && notify-send \"Speaker\" \"Increased volume. \n Current volume: $(pactl get-sink-volume 0 | grep -o -E '(\\w*)\\%' | head -1)\""
decreaseVolume = spawn "pactl set-sink-volume 0 -5% && notify-send \"Speaker\" \"Decreased volume. \n Current volume: $(pactl get-sink-volume 0 | grep -o -E '(\\w*)\\%' | head -1)\""
...
myKeys conf@(XConfig {XMonad.modMask = modm}) = M.fromList $
    ...
    -- Disabling mod-p and mod-shift-p bindings
    -- , ((modm,               xK_p     ), centerlaunch)
    -- , ((modm .|. shiftMask, xK_p     ), ewwclose)

    -- Adding bindings for PostmanAgent and dockerd
    , ((modm,               xK_p     ), spawn "notify-send 'Postman' 'Starting postman agent' -i postman && PostmanAgent")
    -- Dockerd needs sudo access to start. Starting dockerd without sudo has a long enough tutorial that I will go through at a later time. (read never)
    -- , ((modm,               xK_d     ), spawn "dockerd")
    ...
    , ((0,                    xF86XK_AudioRaiseVolume), increaseVolume)
    , ((0,                    xF86XK_AudioLowerVolume), decreaseVolume)
    , ((0,                    xF86XK_AudioMute), muteSpeaker)
    , ((modm .|. shiftMask,   xK_f), muteMic)
...
--     delta   = 3/100
-- Removing Mirror tiled as its useless
...
--  spawnOnce "exec ~/bin/eww daemon"
  ...
  spawnOnce "feh --bg-scale ~/wallpapers/cat.png"
